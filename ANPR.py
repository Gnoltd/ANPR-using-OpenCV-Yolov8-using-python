        ...
        while True:
                ...
                for d in dets:
                    ...
                    if not do_ocr and last_boxes:
                        for (bb_old, txt_old) in last_boxes:
                            if iou(bb, bb_old) >= iou_thres_keep:
                                reused = txt_old
                                break
            ...
            if show_window:
                ...
                if key in (27, ord('q')):
                    break
                if key == ord('s'):
                    snap_dir = Path(SAVE_DIR)
                    snap_dir.mkdir(parents=True, exist_ok=True)
                    fn = snap_dir / f"snapshot_{idx:06d}.jpg"
                ...
                for plate in texts:
                    if plate:
                        ...
                        if (idx - last_seen) > debounce_frames:
                            try:
                                info = lookup_owner(plate)
                                owner = info["owner_name"] if (info and info.get("owner_name")) else "Unknown"
                            except Exception:
                                owner = "Unknown"
...
# run_video is an alias kept for backward compatibility
run_video = run_webcam